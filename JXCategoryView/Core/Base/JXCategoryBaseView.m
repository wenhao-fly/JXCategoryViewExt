...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(JXCategoryCellSelectedType)selectedType {
    return [self selectCellAtIndex:targetIndex selectedType:selectedType animated:YES];
}

- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(JXCategoryCellSelectedType)selectedType animated:(BOOL)animated {
...
//    }
    ...
    if (self.isCellWidthZoomEnabled) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.selectedAnimationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:animated];
        });
    } else {
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:animated];
    }
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (!(self.lastContentViewContentOffset.x == contentOffset.x && self.selectedIndex == baseIndex)) {
            if (self.isSelectItemOnScrollHalf && self.selectedIndex == baseIndex) return;
        ...
        if (fabs(ratio - self.selectedIndex) > 1) {
            ...
            if (self.isSelectItemOnScrollHalf && self.selectedIndex == targetIndex) return;
